...
parser = argparse.ArguentParser(desription = 'This is a custom gpt')
...
# Define the batch function to pick batches from complete text using batch and block size hyper-para
def get_batch(split):
    data = train_data if split=="train" else val_data
    ix = torch.randint(len(data)-block_size,(batch_size,))  # Get random index integers
    x = torch.stack([data[i:i+block_size] for i in ix])  # Stack the text emb created using random index
...
# Define the loss function to estimate loss at certain iterations
